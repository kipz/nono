...
use nono_proxy::token::{
    BARE_NONCE_LEN, BARE_NONCE_PREFIX, PhantomTemplate, find_bare_nonce, rewrite_first_phantom,
};
use rand::RngExt;
...
impl TokenBroker {
    ...
    ///
    /// The full visible string is the store key either way — nothing extra is
    /// appended, or a prefix-sniffing client would misclassify the phantom.
    fn issue_templated(
        &mut self,
    ...
    pub(crate) fn store_named(
        ...
        self.nonce_names.insert(phantom.clone(), name);
        phantom
    }
    ...
    pub(crate) fn issue_named(&mut self, name: &str) -> Option<String> {
        ...
        self.nonce_names.insert(phantom.clone(), name.to_string());
        Some(phantom)
    }
    ...
    pub(crate) fn resolve_nonce_for_credentials(
        ...
        if find_bare_nonce(nonce) != Some((0, BARE_NONCE_LEN)) {
            return None;
        }
        ...
        while i < input.len() {
            ...
            if let Some((len, real, grants, template)) = self.templated_phantom_at(input, i) {
                ...
                let name = std::str::from_utf8(&input[i..i.saturating_add(len)])
                    .ok()
                    .and_then(|phantom| self.nonce_names.get(phantom).cloned());
                let new_phantom = self.issue_templated(real, grants, Some(&template));
                if let Some(name) = name {
                    self.nonce_names.insert(new_phantom.clone(), name);
                }
...
mod tests {
    ...
    fn issue_and_resolve_env_entry() {
        ...
        assert_eq!(
            ...
            "issued nonce must be well-formed"
        );
