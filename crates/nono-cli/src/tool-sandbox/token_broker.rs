...
impl TokenBroker {
    ...
    pub(crate) fn store_named(
        ...
        let phantom = self.issue_templated(zeroized, grants, template.as_ref());
        self.phantom_names.insert(phantom.clone(), name);
        phantom
    }
    ...
    pub(crate) fn issue_named(&mut self, name: &str) -> Option<String> {
        ...
        let phantom = self.issue_templated(value, grants, template.as_ref());
        self.phantom_names.insert(phantom.clone(), name.to_string());
        Some(phantom)
    }
    ...
    /// As [`Self::rewrite_header_value`], but authorised by credential name
    /// rather than by the phantom's grant set.
    ...
    pub(crate) fn scan_and_reissue(&mut self, input: &[u8]) -> Vec<u8> {
        ...
        while i < input.len() {
            ...
            if let Some((len, phantom, template)) = self.templated_phantom_at(input, i)
                && let Some((real, grants)) = self.map.get(&phantom).cloned()
            {
                let name = self.phantom_names.get(&phantom).cloned();
                let new_phantom = self.issue_templated(real, grants, Some(&template));
                self.inherit_credential_name(&new_phantom, name);
            ...
            {
                // Re-issue a fresh phantom for the real value, inheriting grants
                let name = self.phantom_names.get(s).cloned();
                let new_phantom = self.issue_granted(real, grants);
                self.inherit_credential_name(&new_phantom, name);
                out.extend_from_slice(new_phantom.as_bytes());
                i = i.saturating_add(BARE_NONCE_LEN);
                continue;
            }
    ...
    /// Carry a credential name onto a reissued phantom so `redeem_phantoms`
    /// still resolves it by name.
    fn inherit_credential_name(&mut self, phantom: &str, name: Option<String>) {
        if let Some(name) = name {
            self.phantom_names.insert(phantom.to_string(), name);
        }
    }

    /// If a known templated phantom starts at `offset` in `input`, return its
    /// byte length, the phantom itself, and the template that matched it.
    fn templated_phantom_at(
        &self,
        input: &[u8],
        offset: usize,
    ) -> Option<(usize, String, PhantomTemplate)> {
        for template in &self.templates {
            let Some(end) = template.matches_at(input, offset) else {
                continue;
            };
            let Ok(phantom) = std::str::from_utf8(input.get(offset..end)?) else {
                continue;
            };
            if self.map.contains_key(phantom) {
                return Some((
...
mod tests {
    ...
    fn reissued_raw_value_keeps_credential_name() {
        ...
        let nonce = reissued
            .split_whitespace()
            .find(|w| find_bare_nonce(w) == Some((0, BARE_NONCE_LEN)))
    ...
    fn scan_and_reissue_reissues_phantom_shorter_than_a_bare_nonce() {
        // A bare `{}` template renders shorter than `nono_<64hex>`, so the
        // fast-path length floor must account for templates.
        let mut broker = TokenBroker::new();
        let template = PhantomTemplate::parse("{}").unwrap();
        let phantom = broker.store_named(
            "anthropic".to_string(),
