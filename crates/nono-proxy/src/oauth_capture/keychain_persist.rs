...
pub(super) const OAUTH_CAPTURE_ACCOUNT: &str = "oauth_capture_store";
...
/// entry exists, the entry is stale (binary-path mismatch), or the keychain
/// is locked (best-effort: the caller falls back to in-memory-only).
pub(super) fn load_persisted_tokens() -> Result<HashMap<String, StoredOAuthToken>> {
    ...
    let envelope: KeychainEnvelope = serde_json::from_str(&raw).map_err(|err| {
        ProxyError::Keystore(format!("failed to parse OAuth capture keychain entry: {err}"))
    })?;
    ...
    let store_raw = serde_json::to_vec(&envelope.store).map_err(|err| {
        ProxyError::Keystore(format!("failed to re-encode OAuth capture keychain entry: {err}"))
    })?;
...
pub(super) fn persist_tokens(tokens: &HashMap<String, StoredOAuthToken>) -> Result<()> {
    ...
    let payload = Zeroizing::new(serde_json::to_string(&envelope).map_err(|err| {
        ProxyError::Keystore(format!("failed to encode OAuth capture keychain entry: {err}"))
    })?);
...
fn locked_keychain_message(op: &str, service: &str, account: &str, status: i32) -> String {
    format!(
        "OAuth capture store {op} for {service}/{account} blocked: keychain is locked \
         ...
         unlock, or Keychain Access set to lock on inactivity. Persistence is \
         unavailable until the login keychain is unlocked; this session falls back \
         to in-memory-only capture."
    )
}
...
mod tests {
    ...
    fn envelope_round_trips_through_json() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(decoded.nono_path.as_deref(), Some("/opt/shadowfax/bin/nono"));
        assert_eq!(decoded.store, store);
        Ok(())
    }

    #[test]
    fn missing_nono_path_deserialises_as_none() -> std::result::Result<(), Box<dyn std::error::Error>> {
