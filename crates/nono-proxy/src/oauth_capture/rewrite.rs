...
use crate::token::{BARE_NONCE_PREFIX, NonceResolver};
...
impl OAuthCaptureStore {
    ...
    fn contains_phantom(&self, body: &[u8]) -> bool {
        if body
            .windows(BARE_NONCE_PREFIX.len())
            .any(|window| window == BARE_NONCE_PREFIX.as_bytes())
        {
            return true;
        }
        (0..body.len()).any(|offset| {
            self.templates
                .iter()
                .any(|template| template.matches_at(body, offset).is_some())
        })
    }
}
