...
pub(crate) fn resolve_nonce_in_header_value(
    ...
    // Templated OAuth-capture phantoms carry no `nono_` marker, so the bare-nonce
    // scan below never matches them. When the header holds no bare nonce, let the
    // resolver recognise its own templated phantom (grant-set / consumer path).
    if !value.contains("nono_")
        && allowed_credentials.is_empty()
        && let Some(rewritten) = resolver.rewrite_header_value(value, consumer)
    {
