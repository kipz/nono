...
pub trait NonceResolver: Send + Sync {
    ...
    /// rather than by the phantom's grant set. Resolvers that mint templated
    /// phantoms override it to pass their templates to [`rewrite_first_phantom`].
    fn rewrite_header_value_for_credentials(
        ...
        rewrite_first_phantom(value, &[], |nonce| {
            self.resolve_for_credentials(nonce, allowed_credentials)
        })
    }
...
pub fn rewrite_first_phantom(
    ...
    for (start, end) in phantom_spans(value, templates) {
        ...
        {
            let start = jwt_shape_start(value, &value[start..end], start);
...
/// Start of the JWT-shaped phantom wrapping the nonce at `start`, or `start`
/// itself. Substituting only the signature segment would leave the shaped
/// header and payload in front of the real token, yielding five segments.
fn jwt_shape_start(value: &str, nonce: &str, start: usize) -> usize {
    let Ok(shaped) = crate::jwt_phantom::jwt_shaped_phantom(nonce) else {
        return start;
    };
    let Some(prefix) = shaped.strip_suffix(nonce) else {
        return start;
    };
    let Some(shaped_start) = start.checked_sub(prefix.len()) else {
        return start;
    };
    // `.get` (not indexing): shaped_start may fall mid-UTF-8 on a crafted header.
    if value.get(shaped_start..start) == Some(prefix) {
        shaped_start
    } else {
        start
    }
