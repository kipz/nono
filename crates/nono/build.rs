...
//!    `src/bpf/exec_filter.bpf.c` to BPF bytecode and emit a Rust
...
#[cfg(all(target_os = "linux", feature = "bpf-lsm"))]
fn generate_exec_filter_skeleton() {
    use libbpf_cargo::SkeletonBuilder;

    println!("cargo:rerun-if-changed=src/bpf/exec_filter.bpf.c");
    println!("cargo:rerun-if-changed=src/bpf/vmlinux.h");

    let out_dir = env::var("OUT_DIR").expect("OUT_DIR not set");
    let skel_out = Path::new(&out_dir).join("exec_filter.skel.rs");
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set");
    let bpf_dir = Path::new(&manifest_dir).join("src").join("bpf");

    SkeletonBuilder::new()
        .source(bpf_dir.join("exec_filter.bpf.c"))
        ...
        .expect("failed to build BPF exec filter skeleton");
}
