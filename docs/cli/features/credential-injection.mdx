...
<Card title="OAuth token lifecycle (MCP & Claude.ai)" icon="refresh-cw" href="/cli/features/mcp-oauth">
  For a worked example covering OAuth refresh, browser re-login flows, and per-provider token caching, see the OAuth credentials guide.
</Card>
