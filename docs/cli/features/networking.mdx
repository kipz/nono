...
By default the sandboxed process has unrestricted network access. Domain and credential policies route traffic through an HTTP proxy running in the unsandboxed supervisor process. Plain `--block-net` instead installs a static kernel filter and does not start a proxy.
