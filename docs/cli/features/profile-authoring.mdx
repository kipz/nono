...
For agents that talk to MCP servers or the Claude.ai API using short-lived OAuth access tokens, combine `cmd://` capture with a host-side token-cache helper. The helper manages refresh and browser re-login; the proxy injects only the live token into upstream requests while the sandbox is denied access to the token file. See [OAuth Credentials (MCP & Claude.ai)](/cli/features/mcp-oauth) for complete profile examples and a reusable bash helper skeleton.
