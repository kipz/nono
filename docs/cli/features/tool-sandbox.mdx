...
| `allow_writable_executables` | boolean | Profile-wide trust downgrade for writable executable paths. |
...
| `allow_writable_executable` | boolean | Per-command trust downgrade for a writable pinned executable. Requires absolute `executable`. |
