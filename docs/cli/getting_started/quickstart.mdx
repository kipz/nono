...
The `why` command checks if a filesystem path, network operation, kernel scope, or ETI command invocation would be allowed or denied. It's designed for both human debugging and programmatic use by AI agents.
...
# Check an ETI command/argv policy denial
