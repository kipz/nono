...
Trace a command to discover required filesystem paths. Monitors file accesses and outputs paths that would need to be allowed in a nono profile. Supported on Linux (via `strace`) and macOS (via LLDB).
...
<Note>
  `nono learn` runs WITHOUT sandbox restrictions to accurately trace file accesses. Linux requires `strace`; macOS requires Xcode Command Line Tools (`xcode-select --install`).
</Note>
