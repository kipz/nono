...
See [Standalone Proxy](/cli/features/networking#standalone-proxy-nono-proxy) for details.
...
On Linux this always installs a static seccomp filter that denies non-Unix sockets and `io_uring_setup()`, with Landlock layered underneath. This closes Landlock's UDP gap, but also breaks programs using io_uring only for file I/O. Local Unix-domain IPC remains available subject to its normal grants.
...
nono run --profile nolabs-ai/openclaw --listen-port 18789 \
...
- **`--block-net`**: The TCP port becomes an exception to the block
...
**Linux limitation:** Landlock ABI v4+ enforces connect and bind by port but **cannot filter by destination or bind address**. `--open-port 3000` permits connecting to port 3000 on any IP, not just localhost, and permits binding port 3000 on any local interface. `--block-net` blocks other ports but does not narrow the granted port to loopback. Use proxy mode when destination-host enforcement is required. On kernels without Landlock V4, port exceptions fail closed.
</Warning>
...
The value is a plain count (minimum `1`) and counts threads as well as processes, so leave headroom for the threads your workload uses. Combine it with `--memory` freely.

Both resource flags share the same enforcement path:
...
nono run -p nolabs-ai/openclaw -- openclaw gateway
```
...
Refuse to start, instead of warning, when a filesystem grant (`--allow`/`--write`/a profile's `filesystem.allow`) overlaps the ambient `PATH` — either a whole directory, or one binary inside an otherwise-unwritable directory via a file-scoped grant (e.g. `filesystem.write: ["/usr/local/bin/gh"]`).

This isn't about nono's own credential/browser brokers — they already sanitize `PATH` before resolving anything by bare name (see [PATH Sanitization for Host-Side Brokers](/cli/internals/security-model#path-sanitization-for-host-side-brokers)). It's about the writable path itself: once the sandboxed process can write there, it can plant (or overwrite) a same-named binary that anything *else* on the host — a shell, a cron job, an unrelated tool — will run with full privileges the next time it does an ordinary `PATH` lookup, entirely outside nono.

By default nono only warns about this and proceeds, since directories like `$HOME/go/bin` are a common and often legitimate grant. `--strict-broker-path` turns that warning into a hard failure for sessions where you want to rule out the exposure entirely.

```bash
nono run --allow "$HOME/go/bin" --strict-broker-path -- my-agent
```

<Note>
  Not available on `nono wrap`. Warnings are unconditional there; only the strict refusal is opt-in, and `wrap` keeps a minimal flag surface.
</Note>
...
| Value | Behaviour |
|-------|-----------|
| `auto` | Landlock plus a static seccomp baseline for restricted networking, with automatic fallback when the kernel ABI lacks network support (< v4). **Default.** |
| `landlock` | Landlock only. Returns an error at startup if the running kernel cannot satisfy TCP restrictions via Landlock alone. Because Landlock network rules cover TCP only, this explicit mode does not block UDP. |
| `external` | Delegate TCP network lockdown to the deployment. nono still installs filesystem/process sandboxing, but skips its own localhost/proxy-port network enforcement. Use only when infrastructure such as iptables, cgroups, systemd units, or container policy forces egress through the proxy. |

```bash
# Require Landlock-only enforcement; fail if the kernel can't satisfy it
nono run --sandbox-policy landlock --allow-cwd -- my-agent

# Declare that network enforcement is external; nono still applies FS/process sandboxing
